          ...
          <div>
            ...
            <div style={{ 
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '15px' }}>
                <div>
                  <strong>Alert Time:</strong> {userData.initialAssessment?.alert_time?.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}
                </div>
                <div>
                  <strong>Work Style:</strong> {userData.initialAssessment?.project_approach?.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}
                </div>
                <div>
                  <strong>Learning Style:</strong> {userData.initialAssessment?.learning_style?.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}
                </div>
                <div>
                  <strong>Exercise Frequency:</strong> {userData.initialAssessment?.exercise_frequency?.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}
                </div>
              </div>
            </div>
          </div>
